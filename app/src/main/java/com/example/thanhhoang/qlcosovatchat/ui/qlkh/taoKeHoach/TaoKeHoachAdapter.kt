    ...
    inner class TaoKeHoachVH(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun onBind(thietBi: ThietBi) {
            itemView.run {
                ...
                tvDonGiaItemTmkh.text = FormatUtils.formatNumber(thietBi.unitPrice)
                tvThanhTienItemTmkh.text = FormatUtils.formatNumber((thietBi.soLuong?.times(thietBi.unitPrice)) as Long)
            }
        }
    }
}
